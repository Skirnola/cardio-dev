...
import {
  ...
  isExerciseId,
  normalizeExerciseId,
  parseExerciseIdsFromMarkdown,
  type ConceptsFile,
  type Exercise,
  type LearnerExerciseState,
  type LearnerState,
  type ManifestShape,
...
async function main(): Promise<void> {
  ...
  for (const result of results) {
    console.log(
      `- ${result.id}: public=${formatOutcome(result.publicOutcome)} boss=${formatOutcome(result.bossOutcome)} overall=${result.overallPass ? 'pass' : 'fail'}`,
    );
  }
...
type TestOutcome = 'pass' | 'fail' | 'missing';

type VerificationResult = {
  id: string;
  publicOutcome: TestOutcome;
  bossOutcome: TestOutcome | 'not-applicable';
  overallPass: boolean;
  trackedFiles: string[];
};
...
async function resolveExerciseIds(args: VerificationArgs): Promise<string[]> {
...
function resolveTestFiles(filePaths: string[]): string[] {
  return filePaths
...
function runVitest(
  ...
  const vitestArgs = ['--no-install', 'vitest', 'run', '--config', 'vite.config.ts', ...filePaths.map((filePath) => relative(repositoryRoot, filePath))];
...
function applyVerificationUpdates(
  selected: Exercise[],
...
function updateConceptMastery(
  exercise: Exercise,
  passed: boolean,
  learnerState: LearnerExerciseState,
  concepts: ConceptsFile,
): void {
...
function formatOutcome(outcome: VerificationResult['publicOutcome'] | VerificationResult['bossOutcome']): string {
  return outcome;
}
