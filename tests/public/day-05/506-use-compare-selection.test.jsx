...
function CompareSelectionHarness({ initialIds = [], productIds }) {
    ...
    return (<section>
      ...
      {selectedIds.length === 0 ? (<p>No selected items.</p>) : (<ul aria-label="Selected ids">
          {selectedIds.map((id) => (<li key={id}>{id}</li>))}
        </ul>)}
      <div>
        {productIds.map((id) => (<button key={id} type="button" aria-pressed={isSelected(id)} onClick={() => toggleSelected(id)}>
            {id}
          </button>))}
      </div>
    </section>);
