...
import { loadSearchOptionsWithSignal, } from "../../../src/exercises/day-04/036-load-search-options-with-signal";
...
describe("loadSearchOptionsWithSignal", () => {
    it("rejects immediately when the signal is already aborted", async () => {
        ...
        await expect(loadSearchOptionsWithSignal(fetchUsers, "al", controller.signal)).rejects.toBe(reason);
        expect(fetchUsers).not.toHaveBeenCalled();
    });
    it("passes the signal through and resolves normalized options", async () => {
        const controller = new AbortController();
        const fetchUsers = vi.fn(async (_query, signal) => {
        ...
        await expect(loadSearchOptionsWithSignal(fetchUsers, " al ", controller.signal)).resolves.toEqual([
            { id: "u1", label: "Alice" },
            { id: "u2", label: "Sally" },
        ]);
    ...
    it("rejects with the abort reason when the signal aborts before the loader settles", async () => {
        ...
        const fetchUsers = vi.fn((_query, _signal) => deferred.promise);
